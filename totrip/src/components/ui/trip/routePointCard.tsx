    ...
    return (
        <div className={`flex border-l-[2px] border-black relative left-[20px] ${additionalClasses}`}>
            <div className="relative right-[22.5px]">
                <span className="">
                    <Image src={tagImg} alt="Tag" width={44} height={44} className="p-[6.67px] bg-white invert rounded-[100px]" />
                </span>
            </div>
            
            <div className="ml-[96px]">
                <Image src={placeImg} alt={placeName} width={296} height={201} className="rounded-[20px]"/>
            </div>
            <div className="ml-[18px]">
                <div className="flex items-center">
                    ...
                    <ul className="flex ml-[20px] gap-[4.8px]">
                        {rating.map((_, index) => (
                            <li key={index}>
                                <Image src="/img/profile/Star.svg" alt="Рейтинг" width={26} height={26} />
                            </li>
                        ))}
                    </ul>
                </div>
                <p className="mt-[7px] text-[20px] font-[600]">{description}</p>
            </div>
        </div>
    );
};
