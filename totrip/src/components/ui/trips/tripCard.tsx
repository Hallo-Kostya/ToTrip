...
const TripCard: React.FC<TripCardProps> = ({ tripImage, title, description, cities, tripPlace, start_Date, end_Date }) => {
            ...
            <div className="trip-info flex flex-col absolute bottom-[54px] left-[42px]">
                ...
                <div className="trippers flex mt-[20px] gap-[16px]">
                    {Array.from({ length: 1 }).map((_, index) => (
                        <Image key={index} src="" width={52} height={52} alt={`Пользователь ${index + 1}`} className="rounded-full" />
                    ))}
                </div>
            </div>
        </div>
